...
Revision 005:
- Done: solved division by zero problem in update_model_based(self, input)
- Done: added model based controller function: update_model_based(self, input)
...
- Unstable. smoothing of measurement noise. Calculates the mean of past measurements as the input to the controller
...
class FeedbackController(object):
  ...
  def __init__(
    ...
    self.magnetization_curve_forward = magnetization_curve_forward
    self.magnetization_curve_reverse = magnetization_curve_reverse
 ...
 # Sigmoid function along which BH parameter is defined
  ...
  def update_model_based(self, input):
      """
      Update the model based feedback controller. Call the feedback controller.update_model_based() with *input_*
      to calculate and return a control output.
      """      
      # calculate inverse model
      if input > 0:
          magnetization_curve = self.magnetization_curve_forward
      else:
          magnetization_curve = self.magnetization_curve_reverse
      
      idx = self.find_nearest(magnetization_curve, input)
      # Note: delta actuation must be a positive value
      delta = magnetization_curve[idx+1]-magnetization_curve[idx]
      A = abs(delta)
      if A:
          Ai = 1 / A
      else:
      ...
      # compute the proportional term
      proportional = Ai * self.Kp * error

      # compute the integral term
      dt = self.sample_time

      self._integral += Ai * self.Ki * error *dt
      self._integral = self.clamp(self._integral, self.output_limits)  # avoid integral windup

      # compute final output
      if self.output_enabled:
          output = proportional + self._integral
          output = self.clamp(output, self.output_limits)
      else:
          output = 0
